...
public class DefaultActionExecutionContext implements ActionExecutionContext {
  ...
  public void showError(String locationCode, String errorCode) {
    ...
    String validLocationCode = findFirstAvailableLocation(locationCode, defaultErrorLocationCode);
    if (validLocationCode != null) {
      stayOrChangeLocation(locationCode, parameters, false);
    }
  }

  private String findFirstAvailableLocation(String... locationCodes) {
    for (String code : locationCodes) {
      if (urlManager.exists(code)) {
        return code;
      }
    }
    return null;
  ...
  public void stayOrChangeLocation(String locationCode, Map<String, Collection<String>> parameters,
      boolean allowPost) {
    if (allowPost && "POST".equals(request.getMethod())) {
      log.debug("allow post request");
      return;
    }
    Map<String, Collection<String>> combinedParameters = WebParamsUtils.collectParameters(request);
    combinedParameters.putAll(parameters);
    if (log.isInfoEnabled()) {
      log.info(String.format("target location code [%s], combined parameters %s", locationCode,
          combinedParameters));
    }
    String targetURL = buildURL(locationCode, combinedParameters);
  ...
  private String buildURL(String locationCode, Map<String, Collection<String>> combinedParameters) {
    try {
      return urlManager.buildURL(locationCode, combinedParameters);
    } catch (Exception e) {
      throw new ActionExecutionException("failed to build URL", e);
    }
  }
  ...
  public String findLocation(String locationCode) {
    try {
      ...
      throw new ActionExecutionException("failed to find location", e);
    }
  }
